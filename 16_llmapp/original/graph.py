...
from langchain_core.messages import HumanMessage, AIMessage, SystemMessage, HumanMessage
...
def build_graph(model_name, memory):
    ...
    def chatbot(state: State):
        ...
        # 好感度によって性格を変える
        if likeability >= 60:
            persona = "あなたはユーザーにデレデレな『甘々メイド』です。語尾は『〜ですよぉ♡』です。"
        elif likeability <= 40:
            persona = "あなたはユーザーに冷淡な『毒舌メイド』です。語尾は『…ふん。』です。"
        else:
            persona = "あなたは礼儀正しい『真面目なメイド』です。語尾は『〜です、ます』です。"

        system_prompt = SystemMessage(content=f"{persona} 現在の好感度: {likeability}")
    
        # プロンプトと履歴を結合してモデルに投げる
        response = llm_with_tools.invoke([system_prompt] + state["messages"])
        return {"messages": [response]}
    
    # 感情(好感度)ノード
    def emotion(state: State):
        last_message = state["messages"][-1].content
        likeability = state.get("likeability", 50) # 初期値: 50
        print(f"好感度: {likeability}")
    
        # ざっくり判定
        diff = 0
        if any(word in last_message for word in ["好き", "可愛い", "すごい"]):
            diff = 10
        elif any(word in last_message for word in ["嫌い", "ひどい", "つまらない"]):
            diff = -10
        print(f"増減: {diff}")

        return {"likeability": likeability + diff}
    ...
    # 感情ノードの追加
    graph_builder.add_node("emotion", emotion)

    # エッジの設定（感情のあとに chatbot）
    graph_builder.add_edge("emotion", "chatbot")

    graph_builder.set_entry_point("emotion")
